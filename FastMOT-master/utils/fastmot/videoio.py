...
class VideoIO:
    def __init__(self, size, input_uri, camera_num,
                 output_uri=None,
        ...
        self.protocol, self.input_uri = self._parse_uri(self.input_uri, self.camera_num, self.input_uri)
        ...
        # self.source.set(cv2.CAP_PROP_POS_FRAMES, 3000)
    ...
    def get_frame_time(self):
        if self.camera_num == '113':
            strTime = "2022-02-14 11:15:00"
        if self.camera_num == '106':
            strTime = "2022-02-14 11:30:01"

        start_time = datetime.datetime.strptime(strTime, "%Y-%m-%d %H:%M:%S")

        milliseconds = self.source.get(cv2.CAP_PROP_POS_MSEC)
        seconds = milliseconds//1000
        # milliseconds = milliseconds%1000

        frame_time = (start_time + datetime.timedelta(seconds=seconds)).strftime("%Y-%m-%d %H:%M:%S")
        return frame_time
    ...
    def _gst_cap_pipeline(self):
        ...
        if 'nvvidconv' in gst_elements and self.protocol != Protocol.V4L2:
            ...
            print("Gst does not contain nvvidconv")
            cvt_pipeline = (
                'videoscale ! '
        ...
        print("Complete pipeline ->",pipeline+cvt_pipeline)
    ...
    @staticmethod
    def _parse_uri(uri, camera_num, input_uri):
        result = urlparse(uri)
        ...
        elif result.scheme == 'edge':
            input_uri = "rtsp://admin:edge1234@192.168.1." + camera_num + ":554/cam/realmonitor?channel=1&subtype=0"
            protocol = Protocol.RTSP
            # if camera_num == '113':            
            #     input_uri = "rtsp://admin:pass@10.15.40.11/stream0"
            # elif camera_num == '106':            
            #     input_uri = "rtsp://admin:pass@10.15.16.22/stream0"
            # elif camera_num == '115':            
            #     input_uri = "rtsp://admin:P@ssw0rd1234@10.15.56.152/media/video2"
        else:
