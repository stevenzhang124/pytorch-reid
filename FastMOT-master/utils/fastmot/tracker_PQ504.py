				...
				if track.confirmed and self.camera == "106":
					if (x <= 100 and y >= 200):
						...
						if self.database:
							...
							self.entries += 1
				if track.confirmed and self.camera == "113":
					if (x <= 220 and y <= 200):
						LOGGER.info(f"{'Exit_A2:':<14}{track}")
						cmd = "play leave_door.wav"
	...
	def update(self, frame, frame_id, detections, embeddings):
		...
		with Profiler('rlost'):   
			...
			for trk_id, det_id in lost_matches:
				...
				if self.camera == "113":
					# if (x <= 220 and y <= 170):
					if (x <= 220 and (y >= 50 and y <= 200)):
						LOGGER.info(f"{'Enter_A:':<14}{track}")
						cmd = "play enter_door.wav"
						subprocess.Popen(cmd, shell=True)
						if self.database:
							self.database.update_entry(1)
							self.entries = self.database.get_entry()
		...
		with Profiler('rdb'):   
			...
			for trk_id, det_id in db_matches:
				...
				track.reinstate(frame_id, det.tlbr, state, embeddings[det_id])
				self.tracks[trk_id] = track
				self.tracks[trk_id].direction = 1
				x, y = get_center(track.tlbr)
				if self.camera == "113":
					# if (x <= 220 and y <= 170):
