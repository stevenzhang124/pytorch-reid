...
def draw_doors(frame, flag, camera):
    ...
    # door2_ltrb = (5, 130), (100, 415) #117

    # door1_ltrb = (0, 150), (100, 350)
    ...
    if flag > 0:
        if camera == "113":
            ...
            # x, y = get_center(ltrb_to_tlbr(door1_ltrb))
            # text_origin = (int(y - text_size[0] / 2), int(x + text_size[1] / 2))
            # cv2.rectangle(frame, door2_ltrb[0], door2_ltrb[1], color1, -1) # Door 1
            # cv2.putText(frame,TEXT, text_origin, TEXT_FACE, TEXT_SCALE, color2, TEXT_THICKNESS, cv2.LINE_AA )
        ...
        if camera == "106":
            ...
            door1_ltrb = (0, 200), (100, 400)
            # door2_ltrb = (550, 60), (570, 125)

            x, y = get_center(ltrb_to_tlbr(door1_ltrb))
            text_origin = (int(y - text_size[0] / 2), int(x + text_size[1] / 2))
            cv2.rectangle(frame, door1_ltrb[0], door1_ltrb[1], color2, 2) # Door 2
            cv2.putText(frame,TEXT, text_origin, TEXT_FACE, TEXT_SCALE, color2, TEXT_THICKNESS, cv2.LINE_AA )
        ...
        #     cv2.rectangle(frame, door1_ltrb[0], door1_ltrb[1], color1, 2) # Door 2
    else:
        if camera == "113":
            # door1_ltrb = (0, 150), (100, 350)
            door1_ltrb = (120, 50), (190, 150)
            ...
            text_origin = (int(y - text_size[0] / 2), int(x + text_size[1] / 2))
            # cv2.rectangle(frame, door1_ltrb[0], door1_ltrb[1], color2, 2) # Door 1
            # cv2.rectangle(frame, door2_ltrb[0], door2_ltrb[1], color2, 2) # Door 1
            cv2.putText(frame,TEXT, text_origin, TEXT_FACE, TEXT_SCALE, color2, TEXT_THICKNESS, cv2.LINE_AA )
        if camera == "106":
            # pass
            # door1_ltrb = (0, 50), (10, 200)
            ...
            x, y = get_center(ltrb_to_tlbr(door1_ltrb))
            text_origin = (int(y - text_size[0] / 2), int(x + text_size[1] / 2))
            # cv2.rectangle(frame, door1_ltrb[0], door1_ltrb[1], color2, -1) # Door 1
            cv2.putText(frame,TEXT, text_origin, TEXT_FACE, TEXT_SCALE, color2, TEXT_THICKNESS, cv2.LINE_AA )
