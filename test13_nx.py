...
parser.add_argument('-c','--camera', help='Input camera number (eg 106, 115, 117)', required=True)
...
# Init models
reid_mode = cam_reid.reid_model() # osnet_ain_x1_0
...
def draw_box_label(img, bbox_cv2, box_color=(0, 255, 255), personReID_info={'personID':'Unknown'}, show_label=True):
	...
	if show_label:
		...
		top_left = "x1,y1: {},{}".format(str(left),str(top))
		right_bottom = "x2,y2: {},{}".format(str(right),str(bottom))
		cv2.putText(img,top_left,(left,top), font, font_size, font_color, 2, cv2.LINE_AA)
		cv2.putText(img,right_bottom,(left,bottom), font, font_size, font_color, 2, cv2.LINE_AA)
		...
		for idx in unmatched_dets:
			...
			if(identify_name in [ "MJ1","MJ2","MJ3","MJ4","MJ5","MJ6","MJ7","MJ8","MJ9","MJ10","MJ11","MJ12","MJ13","MJ14","MJ15","MJ16","MJ17"]):				
				...
				if int(time.time()) - tmp > 15:
					mydb = db_connection()
					cur = mydb.cursor()
					info = "insert into REID(person, ctime, camera, pos_x, pos_y) values ('{0}',{1},{2},{3},{4})".format(identify_name, int(time.time()), c1, x2, y2)
					tmp = int(time.time())
					cur.execute(info)
				...
				if int(time.time()) - tmp2 > 15:
					...
					print("insert one record", tmp)
					mydb.close()
			print("identify name:{}, score:{}, (x1,y1): {},{} (x2,y2):{},{}".format(identify_name, round(1-score, 2), x1, y1, x2, y2))
...
def gen_frames():  # generate frame by frame from camera
	...
	uri = "rtsp://admin:edge1234@192.168.1." + args.camera + ":554/cam/realmonitor?channel=1&subtype=0"
	# rtsp://admin:edge1234@192.168.1.105:554/cam/realmonitor?channel=1&subtype=0

	print("Camera number {} \nURI is {}".format(args.camera,uri))
	
	cap = open_cam_rtsp(uri, 640, 480, 200) #cv2.VideoCapture()
