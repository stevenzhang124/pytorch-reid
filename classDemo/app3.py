...
required.add_argument('-p', '--port', help='port for similar device', type=int)
...
def execa(mot, stream, args, logger, vid):
    txt = None
    ...
    try:
        ...
        init_time = time.time()
        with Profiler('app') as prof:
            while not args.gui or cv2.getWindowProperty(vid, 0) >= 0:
                frame = stream.read()
                if frame is None:
                    break
    ...
    # Final Times
    dura = prof.duration
    total_f = mot.frame_count
    avg_fps = round(total_f / dura)
    # mot.tracker.getTimes()
    mot.getAverages(avg_fps, logger)

    print("Ela = {} / Frame = {} --> FPS = {}".format(int(elapsed_time), total_f, int(total_f/elapsed_time)))
...
def main():
    ...
    # stream = fastmot.VideoIO(config['size'], config['video_io'], args.input_uri, args.camera, args.output_uri)
    stream = fastmot.VideoIO(config.resize_to, args.input_uri, args.camera, args.output_uri, **vars(config.stream_cfg))
    bstream = fastmot.VideoIO(config.resize_to, args.input_uri, args.camera, args.output_uri, **vars(config.stream_cfg))
    cstream = fastmot.VideoIO(config.resize_to, args.input_uri, args.camera, args.output_uri, **vars(config.stream_cfg))
    ...
    if args.mot:
        ...
        # bmot = fastmot.MOT(config.resize_to, args.camera, db, bstream.cap_fps, args.par, **vars(config.mot_cfg),
        #  draw=draw, verbose=args.verbose)# cuda_ctx=pycuda.autoinit.context)
        # bmot.reset(bstream.cap_dt)
        
        # cmot = fastmot.MOT(config.resize_to, args.camera, db, cstream.cap_fps, args.par, **vars(config.mot_cfg),
        #  draw=draw, verbose=args.verbose)# cuda_ctx=pycuda.autoinit.context)
        # cmot.reset(cstream.cap_dt)
        ...
        threading.Thread(target=execa, args=(mot, stream, args, logger,"a")).start()
        threading.Thread(target=execa, args=(mot, bstream, args, logger,"b")).start()
        threading.Thread(target=execa, args=(mot, cstream, args, logger,"c")).start()

        # multiprocessing.Process(target=execa, args=(mot, stream, args, logger,)).start()
        # multiprocessing.Process(target=execa, args=(mot, bstream, args, logger,)).start()
        # multiprocessing.Process(target=execa, args=(mot, cstream, args, logger,)).start()
...
if __name__ == '__main__':
    with threadpool_limits(limits=1, user_api='blas'):
        ...
        if not args.exclusive:
            app.run(host='0.0.0.0', port=5000+args.port)
        else:
            main()
