    ...
    def _draw(self, frame, detections):
        ...
        if self.tracker.camera == '117':
            ...
            for i, line in enumerate(text.split('\n')):
                y = y0 + i*dy
                cv2.putText(frame, line, (5, y ), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (int((1-occ_room)*255), int((1-occ_room)*255), 255), 1, cv2.LINE_AA)
        else:
            color = (255, 255, 255)
            cv2.putText(frame, 'IN: {}'.format(self.tracker.entries), (5, 20),
                    cv2.FONT_HERSHEY_TRIPLEX, 0.65, color, 1, cv2.LINE_AA)
            cv2.putText(frame, 'OUT: {}'.format(self.tracker.exits), (5, 45),
                    cv2.FONT_HERSHEY_TRIPLEX, 0.65, color, 1, cv2.LINE_AA)
