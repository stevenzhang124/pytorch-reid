    ...
    def apply_kalman(self, frame):
        ...
        for trk_id, track in list(self.tracks.items()):
            ...
            if ios(next_tlbr, self.frame_rect) < 0.5:
                ...
                if track.confirmed and self.camera == "106":
                    if (x <= 100 and y >= 100):
                        LOGGER.info(f"{'Enter_B2:':<14}{track}")
                        if self.database:
        ...
        with Profiler('rfound'):   
            ...
            for trk_id, det_id in matches:
                ...
                if track.hits >= self.confirm_hits:
                    ...
                    if track.hits == self.confirm_hits:
                        # print("Tracks hits",self.tracks)
                        x, y = get_center(track.tlbr)
                        # if x >= 300:
                        if self.camera == "113":
                            if (x <= 220 and y <= 150):
                                LOGGER.info(f"{'Enter_A:':<14}{track}")
                                if self.database:
                                    self.database.update_entry(1)
                                    self.entries = self.database.get_entry()
                                else:
                    ...
                    elif track.hits % min_hits_confirmation == 0:
                        curr_x,curr_y = get_center(track.bboxes[-1])
                        prev_x,prev_y = get_center(track.bboxes[-1-min_hits_confirmation])
                        # LOGGER.info(f"{'Prev (X,Y):':<14}{prev_x,prev_y}")
                        # LOGGER.info(f"{'Curr (X,Y):':<14}{curr_x,curr_y}")
                        # print("############################################################################")
                        if curr_y - prev_y < -5 :
                            track.direction = -1 #Exit
                        elif curr_y - prev_y > 5:
                            track.direction = 1 #Entrance
                        else:
                            track.direction = 0
                    if self.database:
                        if track.hits == 10 :
            ...
            for trk_id in u_trk_ids:
                ...
                if track.age > self.max_age:
                    ...
                    if self.camera == "106":
                        ...
                        if (x <= 100 and y >= 100):
                            LOGGER.info(f"{'Enter_B:':<14}{track}")
                            if self.database:
                                self.database.update_entry(1)
                                self.entries = self.database.get_entry()
                            else:
