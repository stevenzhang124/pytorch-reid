...
class MultiTracker:
    ...
    def initiate(self, frame, detections):
        ...
        for det in detections:
            ...
            self.database.insert_track(new_trk.trk_id, new_trk._to_dict())
        ...
        for trk_id, track in list(self.tracks.items()):
            ...
            if iom(next_tlbr, self.frame_rect) < 0.5:
                if track.confirmed:
                    ...
                    self.database.delete_track(trk_id, self.cameraids[str(self.camera)])
    ...
    def update(self, frame_id, detections, embeddings):
        ...
        print("C = {}, UC = {}".format(confirmed,unconfirmed))
        # print("Confirmed Detections = {}, Embeddings = {}".format(detections[confirmed],embeddings[confirmed]))
        # print("Unconfirmed Detections = {}, Embeddings = {}".format(detections[unconfirmed],embeddings[unconfirmed]))
        ...
        # cost = self._iou_cost(unconfirmed, detections, 1)
        # db_matches, u_trk_ids0, u_det_ids = self._linear_assignment(cost, db_ids, u_detections)
    ...
    def _update_tracks(self):
        ...
        if db_tracks:
            # print("tracks len", len(db_tracks))
            for trck in db_tracks:
                # print("Track exists in tracks -> {}".format(self.isExistTrkID(trck["trk_id"],0)))
                # print("Track exists in db_tracks -> {}".format(self.isExistTrkID(trck["trk_id"],1)))
                if self.isExistTrkID(trck["trk_id"],0):
                    continue
                # print("Track_ID={}/{} will be checked...".format(trck["trk_id"],len(db_tracks)))
                if trck["is_duplicate"] >= self.cameraids[str(self.camera)]:
                    print("Already checked ", trck["trk_id"])
                    continue
                ...
                self.tracks[track.trk_id] = track
    ...
    def _iou_cost(self, trk_ids, detections, dict_type=0):
        dico = self.tracks if (dict_type == 0) else self.db_tracks 
        if len(trk_ids) == 0 or len(detections) == 0:
            return np.empty((len(trk_ids), len(detections)))

        # make sure associated pair has the same class label
        trk_labels = np.array([dico[trk_id].label for trk_id in trk_ids])
        trk_bboxes = np.array([dico[trk_id].tlbr for trk_id in trk_ids])
    ...
    def _remove_duplicate(self, updated, aged):
        ...
        for trk_id in dup_ids:
            ...
            self.database.delete_track(min(aged_id,updated_id), self.cameraids[str(self.camera)],1)
